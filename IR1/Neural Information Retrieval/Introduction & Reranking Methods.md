...
- Score documents based on formula: $score(D, Q) = \sum_{i=1}^{|Q|} IDF(q_i) * TF(q_i, D)$
- Where:
...
4. It creates a similarity matrix between query and document terms using cosine similarity of their embeddings
5. For each query term, it computes a histogram of similarity scores with all document terms
6. These histograms are processed through a feed-forward network to get a relevance score for each query term
7. The final relevance score is a weighted sum of per-term scores

The process works as follows:

8. Load document and embeddings
9. Compute histograms capturing similarity between query and document terms
10. Process histograms through a neural network
11. Weight query terms (often by IDF)
12. Combine scores for the final relevance judgment

DRMM is trained using binary cross-entropy loss: $L = - \sum_{j \in J_{pos}} \log(s_j) - \sum_{j \in J_{neg}} \log(1 - s_j)$
...
- Score is derived from the probability of generating "true"
...
17. **Non-parametric (Retrieval-Based)**:
